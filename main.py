...
# --- SECCIÓN 1: FINANZAS + PRESUPUESTO + HISTORIAL ---
if menu == "💰 FINANZAS IA":
    ...
    with st.expander("⚙️ Configurar Límite de Presupuesto"):
    ...
    with st.form("f", clear_on_submit=True):
        ...
        if st.form_submit_button("REGISTRAR"):
            ...
            conn.execute("INSERT INTO finanzas (tipo, categoria, monto, fecha) VALUES (?,?,?,?)", (t, cat, v, fecha))
            conn.commit(); st.rerun()
    ...
    if not df_f.empty:
        ...
        m2.metric("GASTOS TOTALES", f"RD$ {gastos:,.2f}")
        if pred: m3.metric("PREDICCIÓN IA", f"RD$ {pred:,.2f}", delta=round(pred-bal, 2))
        
        if limite_actual > 0:
            prog = min(gastos / limite_actual, 1.0)
...
# --- SECCIÓN 2: BIOMONITOR + SEMÁFORO PERMANENTE ---
    ...
    if val_g > 0:
        if val_g <= 140: st.markdown(f'<div class="semaforo-verde">🟢 NORMAL: {val_g} mg/dL</div>', unsafe_allow_html=True)
            ...
            for i, (n, num) in enumerate(contactos.items()):
                link = f"https://api.whatsapp.com/send?phone={num}&text=Urgente:%20Soy%20Luis.%20Glucosa%20en%20{val_g}"
...
# --- SECCIÓN 3: AGENDA MÉDICA + GMAIL ---
    ...
    with col1:
        ...
        with st.form("m"):
            nom = st.text_input("Nombre:"); hor = st.text_input("Hora:")
...
# --- SECCIÓN 4: ESCÁNER ---
    ...
    if st.button("📄 GENERAR PDF"):
        ...
        st.success(f"Reporte guardado: {n_p}")
...
# --- CRÉDITOS ---
st.sidebar.markdown("---")
st.sidebar.subheader("🚀 CRÉDITOS DEL PROYECTO")
st.sidebar.write("💎 **SISTEMA QUEVEDO v8.0**")
st.sidebar.info("👨‍💻 **Desarrollador Principal:** \n\n **Luis Rafael Quevedo**")
st.sidebar.caption("© 2026 - Control Integral de Salud y Finanzas")
